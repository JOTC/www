angular.module('jotc').controller('manage-users', [
  '$scope',
  '$http',
  '$modal',
  'jotc-auth',
  function($scope, $http, $modal, $auth) {
    $scope.auth = $auth;
    ...
    $scope.permissions = {
      ...
      payments: 'Can view processed payments'
    };
    ...
    $scope.saveSelectedUser = function() {
      ...
      if ($scope.selectedUserShadow._id) {
        promise = $http
          .put(
            '/data2/users/' + $scope.selectedUserShadow._id,
            $scope.selectedUser
          )
          .success(function() {
            ...
            for (var permission in $scope.selectedUser.permissions) {
              if ($scope.selectedUser.permissions.hasOwnProperty(permission)) {
                $scope.selectedUserShadow.permissions[permission] =
                  $scope.selectedUser.permissions[permission];
              }
            }
          });
      } else {
        promise = $http
          .post('/data2/users', $scope.selectedUser)
    ...
    $scope.deleteSelectedUser = function() {
      if (
        ...
        if (
          ...
          $http
            .delete('/data2/users/' + $scope.selectedUserShadow._id)
            .success(function() {
              for (var i in $scope.users.list) {
                if (
                  $scope.users.list[i]._id === $scope.selectedUserShadow._id
                ) {
