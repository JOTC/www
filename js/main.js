angular
  ...
  .config([
    ...
    function($routeProvider) {
      ...
      $routeProvider
        ...
        .when('/payments', {
          templateUrl: 'jotc/sections/payments/template.html',
          controller: 'payments'
        })
        .when('/resetPassword', {
  ...
  .service('jotc-auth', [
    ...
    function($http) {
      ...
      var getUser = function() {
        $http.get('/data2/auth/user').success(function(user) {
          loggedIn = false;
          userID = '';
          username = '';
          ...
          if (user && user.permissions) {
            ...
            permissions.payments = user.permissions.payments;

            if (permissions.users) {
              $http.get('/data2/users').success(function(list) {
                Array.prototype.splice.apply(
                  users.list,
                  [0, users.list.length].concat(list)
                );
              });
            }
          } else {
    ...
    function() {
      return Object.freeze({
        getImageURLForLocation: function(locationStr) {
          ...
          return (
            'https://maps.googleapis.com/maps/api/staticmap?sensor=false&zoom=11&size=200x150&center=' +
            loc +
            '&markers=' +
            loc
          );
        ...
        getDirectionsURLForLocation: function(locationStr) {
          ...
          return (
            'https://maps.google.com/maps?q=' + locationStr.replace(/ /g, '+')
          );
        }
      });
    }
  ...
  .controller('main', [
    ...
    '$http',
    'jotc-auth',
    function($scope, $window, $modal, $http, $auth) {
      ...
      $scope.logout = function() {
        $http.get('/data2/auth/logout').success(function() {
          $auth.refresh();
        });
      };
